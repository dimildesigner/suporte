...
document.querySelector('#search').addEventListener('input', render); document.querySelector('#status-filter').addEventListener('change', render); document.querySelector('#new-ticket-button').addEventListener('click', () => document.querySelector('#ticket-dialog').showModal()); document.querySelector('#clear-data').addEventListener('click', () => { state.tickets=[]; persist(); render(); });
...
render();const storageKey = 'senai-suporte-tickets';
const list = document.querySelector('#ticket-list');
const state = { tickets: JSON.parse(localStorage.getItem(storageKey) || '[]') };
const labels = { OPEN: 'Aberto', IN_PROGRESS: 'Em atendimento', RESOLVED: 'Resolvido' };
const esc = value => String(value).replace(/[&<>'"]/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' })[character]);
